...
#'
#' This function will perform random contact followup at a given time `t`
#' 
...
random_followup_t <- function(x, t, coverage, followup_only = FALSE) {
...
random_followup_t(x, 10, 0.5)
random_followup_t(x, 10, 0.5) %>%
  count(followup_10)

```

For convenience, we also build a function which will achieve followup for all
days in a given time window:

```{r }

random_followup <- function(x, coverage) {
  ...
  for (i in seq_along(days)) {
    out <- random_followup_t(previous_followup,
                                    t = days[i],
                                    0.01,
                                    FALSE)
...
* the breakdown of cases detected / not detected, and associated proportion
* 
...
Followup for all contacts over the full time period:

```{r }

res_random <- random_followup(x, 0.01)
res_random
plot(res_random)
