...
add_followup <- function(x, followup_duration) {
  mutate(x,
         followup_start = as.integer(exposure_end + 1),
         followup_end = as.integer(followup_start + followup_duration - 1))
}
...
random_followup <- function(x, t, coverage, followup_only = FALSE) {
  ...
  if (!"last_seen" %in% names(out)) {
    out <- mutate(out, last_seen = rep(-1L, n()))
  }
  if (!"detected" %in% names(out)) {
    out <- mutate(out, detected = rep(FALSE, n()))
  }
